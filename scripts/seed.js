...
async function seedUsers(client) {
    try {
        ...
        // Create the "users" table and the enum types if they don't exist
    ...
    try {
        ...
        END $$;
          CREATE TABLE IF NOT EXISTS campaigns (
            ...
            campaign_leader_id UUID,
            status campaign_status NOT NULL DEFAULT 'active',
            timestamp TIMESTAMP NOT NULL DEFAULT NOW(),
            FOREIGN KEY (campaign_leader_id) REFERENCES users(id) ON DELETE SET NULL
          );
        `;
        ...
        const insertedCampaigns = await Promise.all(
            campaigns.map(async (campaign) => {
                const leaderId = campaign.campaign_leader_id || 'some_default_user_id'; // Replace with a valid user ID
                return client.sql`
                    INSERT INTO campaigns (id, name, campaign_leader_id, status, timestamp)
                    VALUES (${campaign.id}, ${campaign.name}, ${leaderId}, ${campaign.status}, ${campaign.timestamp})
                    ON CONFLICT (id) DO NOTHING;
                `;
            })
        );
...
async function seedVolunteersWorksOrWorkedIn(client) {
    try {
        ...
        console.log(`Seeded ${insertedVolunteersWorksOrWorkedIn.length} volunteers_works_or_worked_in`);
...
async function seedRecipientReceiveRelief(client) {
    try {
        ...
        console.log(`Seeded ${insertedRecipientReceiveRelief.length} recipient_receive_relief`);
        ...
        console.error("Error seeding recipient_receive_relief:", error);
        throw error;
    }
}
...
async function seedTeamWorksWithRelief(client) {
    try {
        ...
        console.log(`Seeded ${insertedTeamWorksWithRelief.length} team_works_with_relief`);
